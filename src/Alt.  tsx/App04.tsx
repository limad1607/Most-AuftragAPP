...
export default function App() {
  ...
  const exportCurrentOrder = async () => {
    ...
    const drawLineField = (label: string, value: string, x: number, y: number, labelWidth: number, lineWidth: number) => {
      doc.setFont("helvetica", "normal");
      doc.setFontSize(8.5);
      doc.text(label, x, y);
      doc.line(x + labelWidth, y + 0.2, x + labelWidth + lineWidth, y + 0.2);
      if (value) {
        doc.text(String(value).slice(0, Math.max(8, Math.floor(lineWidth * 1.6))), x + labelWidth + 1.5, y - 0.9);
      }
    };

    const drawCheckbox = (checked: boolean, label: string, x: number, y: number) => {
      doc.rect(x, y - 3.5, 3.5, 3.5);
      if (checked) doc.text("X", x + 0.8, y - 0.5);
      if (label) {
        doc.setFont("helvetica", "normal");
        doc.setFontSize(8.5);
    ...
    drawLineField("Firma:", order.company || "", 12, 24, 16, 86);
    drawLineField("Straße:", order.street || "", 12, 39, 18, 84);
    drawLineField("PLZ:", order.zip || "", 12, 54, 14, 24);
    drawLineField("Ort:", order.city || "", 42, 54, 12, 56);
    drawLineField("Telefon:", order.phone || "", 12, 69, 22, 36);
    drawLineField("Fax:", order.fax || "", 72, 69, 12, 38);

    doc.setFont("helvetica", "bold");
    doc.setFontSize(8.5);
    doc.text("Pflichtdaten", 12, 76);
    drawCheckbox(order.isFirstOrder || (!order.isFollowUp && !order.isPhonePost), "Erstauftrag", 12, 82);
    drawCheckbox(order.isFollowUp, "Folgeauftrag", 38, 82);
    drawLineField("Datum:", order.date || "", 72, 82, 16, 38);
    ...
    doc.text("Most GmbH & Co Kg.", 150, 13);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(8.3);
    [
      "Waldweg 1",
      ...
      "Ust.Id.Nr.: DE",
    ].forEach((line, idx) => doc.text(line, 150, 17 + idx * 4));

    drawLineField("Mitarbeiter:", order.employee || "", 128, 45, 24, 42);
    drawLineField("Kunden Nr.:", order.customerNo || "", 128, 55, 24, 42);
    drawLineField("Kundenbestellnr.:", order.customerOrderNo || "", 128, 65, 32, 34);
    drawLineField("Kommission:", order.commission || "", 128, 75, 23, 43);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(8.5);
    doc.text("Liefertermin", 128, 85);
    drawLineField("Wunsch:", order.deliveryAddress || "", 128, 91, 20, 46);

    const tableX = 12;
    const tableY = 98;
    const rowH = 8.1;
    const col = [12, 22, 46, 58, 139, 159, 177, 196];
    ...
    doc.text("Menge", 48, tableY + 5.5);
    doc.text("Warenbezeichnung", 74, tableY + 5.5);
    doc.text("Einzelpreis", 141, tableY + 5.5);
    doc.text("Gesamtpreis", 160, tableY + 5.5);
    ...
    lines.forEach((line, idx) => {
      ...
      doc.text((line.quantity || "").slice(0, 6), 48, y);
      doc.text((line.description || "").slice(0, 44), 60, y);
      doc.text(money(normalizeNumber(line.unitPrice)), 141, y);
      doc.text(money(lineTotal(line)), 160, y);
    });
    ...
    if (order.signatureDataUrl) {
      try {
        doc.addImage(order.signatureDataUrl, "PNG", 98, footerY + 9.5, 34, 9);
      } catch {}
    }
    doc.setFontSize(8);
    if (order.signatureName) doc.text(order.signatureName.slice(0, 30), 82, footerY + 17.5);
    ...
    doc.setFont("helvetica", "bold");
    doc.setFontSize(8.5);
    doc.text("Summe:", 170, footerY + 12.2, { align: "center" });
    doc.text("Abteilung:", 176, footerY + 23.2, { align: "center" });
    doc.setFont("helvetica", "normal");
    doc.text(money(orderTotal(order)), 176, footerY + 15.6, { align: "center" });
    ...
    if (order.emailInvoice) doc.text(order.emailInvoice.slice(0, 42), 71, 278);
    doc.setFontSize(7.8);
  ...
  const exportDailyReport = async () => {
    ...
    const col = [12, 24, 38, 102, 130, 158, 178, 190];
    const top = 35;
    ...
    doc.line(102, 40.5, 178, 40.5);
    doc.line(130, 46, 130, bodyBottom);
    doc.line(158, 46, 158, bodyBottom);

    doc.setFontSize(7.4);
    doc.text("Fax/W/N", 25, 39.5);
    doc.text("Uhrzeit", 25.2, 43.6);
    doc.text("NAME UND ADRESSE", 63, 41.5);
    doc.text("PRODUKT", 130, 39.5, { align: "center" });
    doc.text("Nachbestellung", 110, 44.2);
    doc.text("Erstbestellung", 138, 44.2);
    doc.text("Summe", 168, 41.5);
    doc.text("Kommentar Cod.", 160.5, 44.2);
    ...
    rows.forEach((row, idx) => {
      ...
      doc.setFont("helvetica", "bold");
      doc.setFontSize(13);
      doc.text(`${idx + 1},`, 18, ry + 11, { align: "center" });
      doc.setFont("helvetica", "normal");
      doc.setFontSize(7.5);
      doc.text(row.code, 30.5, ry + 8.5, { align: "center" });
      const parts = row.nameAddress.split("\n");
      doc.text((parts[0] || "NAME").slice(0, 26), 39, ry + 4.8);
      doc.text((parts[1] || "ADRESSE").slice(0, 26), 39, ry + 11);
      doc.text((parts[2] || "TEL. NR. ODER PERSON").slice(0, 26), 39, ry + 17.2);
      doc.text((row.productRepeat || "").slice(0, 24), 103, ry + 5.5);
      doc.text((row.productFirst || "").slice(0, 24), 131, ry + 5.5);
      doc.text(row.amount ? money(row.amount) : "", 159.5, ry + 17.2);
      doc.text((row.comment || "").slice(0, 16), 160, ry + 11);
    });
    ...
    const drawSummaryTable = (
      ...
      for (let i = 1; i <= 4; i += 1) doc.line(x0, y0 + 7 + i * 9.75, x0 + w, y0 + 7 + i * 9.75);

      doc.setFont("helvetica", "normal");
      doc.setFontSize(7.2);
      doc.text(title, x0 + w / 2, y0 + 5, { align: "center" });
      doc.text("Art", x0 + 4, y0 + 13.5);
      doc.text("Total", x0 + 26, y0 + 13.5);
      doc.text("Summe", x0 + 72, y0 + 13.5, { align: "center" });
      doc.text("Besuche", x0 + 11.5, y0 + 20.5);
      doc.text("Demos", x0 + 26.5, y0 + 20.5);
      doc.text("Verkäufe", x0 + 40.5, y0 + 20.5);
      ...
      rowsLocal.forEach((entry, idx) => {
        const y = y0 + 27 + idx * 9.75;
        doc.text(entry[0], x0 + 2.5, y);
        doc.text(String(entry[1].visits), x0 + 14.5, y, { align: "center" });
        doc.text(String(entry[1].demos), x0 + 28.5, y, { align: "center" });
        doc.text(String(entry[1].sales), x0 + 42.5, y, { align: "center" });
