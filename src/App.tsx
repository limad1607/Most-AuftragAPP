...
const DB_NAME = "most-vite-db-v21";
const DB_VERSION = 2;
...
type ImportedCustomerHistory = {
  ...
  fax: string;
  sourcePdf: string;
  products: ImportedArticleHistory[];
  importedAt: string;
  updatedAt: string;
};

type CustomerHistoryJsonPayload = {
  source_pdf?: string;
  customer_count?: number;
  article_row_count?: number;
...
type JsonImportPreview = {
  ...
  productCount: number;
  customers: ImportedCustomerHistory[];
  products: Product[];
  sampleCustomers: ImportedCustomerHistory[];
};
...
type BackupPayload = {
  ...
  orders: Order[];
  dayPlans?: DayPlan[];
  importedHistories?: ImportedCustomerHistory[];
};
...
function openDb(): Promise<IDBDatabase> {
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = () => {
      ...
      if (!db.objectStoreNames.contains("importedHistories")) db.createObjectStore("importedHistories", { keyPath: "id" });
    };
...
function customerIdentity(item: Pick<Order, "customerId" | "customerNo" | "company" | "street" | "city"> | Pick<Customer, "id" | "customerNo" | "company" | "street" | "city">) {
  const idValue = "id" in item ? item.id : item.customerId;
    ...
    .forEach((order) => {
      order.lines.filter(lineFilled).forEach((line) => {
        ...
        if (hasSale) {
          ...
          entry.unitPrice = normalizeNumber(line.unitPrice) || entry.unitPrice;
          entry.orders += 1;
        }
...
function summarizeImportedHistoryProducts(histories: ImportedCustomerHistory[]): ProductSummary[] {
  const map = new Map<string, ProductSummary>();

  histories.forEach((history) => {
    history.products.forEach((product) => {
      const key = `${String(product.articleNo || "").trim().toLowerCase()}||${String(product.description || "").trim().toLowerCase()}`;
      if (!key || key === "||") return;
      const entry = map.get(key) || {
        key,
        articleNo: String(product.articleNo || "").trim(),
      ...
      const importedQty = normalizeNumber(
        product.quantity ??
          product.qty ??
          product.menge ??
          0,
      );
      const importedAmount = normalizeNumber(
        product.turnover ??
          product.revenue ??
          product.amount ??
          product.totalPrice ??
      ...
      const importedUnitPrice = normalizeNumber(
        product.lastPrice ??
          product.last_price ??
          product.unitPrice ??
          product.unit_price ??
      ...
      entry.quantity += importedQty;
      entry.amount += importedAmount;
      entry.unitPrice = importedUnitPrice || entry.unitPrice;
      entry.orders += 1;
      map.set(key, entry);
    });
  });
...
function mergeProductSummaries(current: ProductSummary[], imported: ProductSummary[]) {
  const map = new Map<string, ProductSummary>();

  [...current, ...imported].forEach((product) => {
    const key = product.key || `${product.articleNo.trim().toLowerCase()}||${product.description.trim().toLowerCase()}`;
    const entry = map.get(key) || {
      key,
    ...
    entry.articleNo = entry.articleNo || product.articleNo;
    entry.description = entry.description || product.description;
    entry.quantity += normalizeNumber(product.quantity || 0);
    entry.amount += normalizeNumber(product.amount || 0);
    entry.unitPrice = normalizeNumber(product.unitPrice || 0) || entry.unitPrice || 0;
    entry.orders += normalizeNumber(product.orders || 0);
    entry.demos += normalizeNumber(product.demos || 0);
    map.set(key, entry);
  });
...
export default function App() {
  ...
  const [importedHistories, setImportedHistories] = useState<ImportedCustomerHistory[]>([]);
  const [historyPreview, setHistoryPreview] = useState<JsonImportPreview | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const loadedImportedHistories = await dbGetAll<ImportedCustomerHistory>("importedHistories");
      ...
      setImportedHistories(loadedImportedHistories.sort((a, b) => String(b.updatedAt || "").localeCompare(String(a.updatedAt || ""))));
  ...
  const importedHistoryMap = useMemo(() => {
    const map = new Map<string, ImportedCustomerHistory[]>();
    importedHistories.forEach((history) => {
      const key = customerIdentity({
        id: history.id,
        customerNo: history.customerNo,
      ...
      if (history.customerNo.trim()) {
        const noKey = `no:${history.customerNo.trim()}`;
        const current = map.get(noKey) || [];
        if (!current.includes(history)) current.push(history);
        map.set(noKey, current);
      }
    });
    return map;
  }, [importedHistories]);

  const selectedCustomerImportedHistories = useMemo(() => {
    if (!selectedCustomer) return [];
    return importedHistoryMap.get(customerIdentity(selectedCustomer)) || importedHistoryMap.get(`no:${selectedCustomer.customerNo.trim()}`) || [];
  }, [selectedCustomer, importedHistoryMap]);

  const selectedCustomerProducts = useMemo(
    () => mergeProductSummaries(
      summarizeCustomerProducts(selectedCustomerOrders),
      summarizeImportedHistoryProducts(selectedCustomerImportedHistories),
    ),
    [selectedCustomerOrders, selectedCustomerImportedHistories],
  );
  ...
  const selectedCustomerTotals = useMemo(() => {
    const importedAmount = selectedCustomerImportedHistories.reduce(
      (sum, history) => sum + history.products.reduce((inner, product) => inner + normalizeNumber(product.turnover || 0), 0),
      0,
    );

    const importedOrders = selectedCustomerImportedHistories.reduce(
      (sum, history) => sum + history.products.length,
      0,
    );

    return {
      orders: selectedCustomerOrders.length + importedOrders,
      amount: selectedCustomerOrders.reduce((sum, item) => sum + (item.total || orderTotal(item)), 0) + importedAmount,
    };
  }, [selectedCustomerOrders, selectedCustomerImportedHistories]);
...
const drawOrderPdfPage = (doc: any, pdfOrder: Order) => {
    const lines = ensureRows(pdfOrder.lines).slice(0, 14);

    const firstChecked = pdfOrder.isFirstOrder || (!pdfOrder.isFollowUp && !pdfOrder.isPhonePost);
    const followChecked = pdfOrder.isFollowUp;
    ...
    drawLineField("Firma:", pdfOrder.company || "", 12, 24, 16, 86, 42);
    drawLineField("Straße:", pdfOrder.street || "", 12, 39, 18, 84, 42);
    drawLineField("PLZ:", pdfOrder.zip || "", 12, 54, 14, 22, 10);
    drawLineField("Ort:", pdfOrder.city || "", 48, 54, 10, 52, 26);
    drawLineField("Telefon:", pdfOrder.phone || "", 12, 69, 22, 34, 18);
    drawLineField("Fax:", pdfOrder.fax || "", 72, 69, 12, 36, 18);
    ...
    drawLineField("Datum:", pdfOrder.date || "", 72, 82, 16, 38, 18);

    drawCheckbox(pdfOrder.orderType === "auftrag", "", 12, 90);
    doc.setFont("helvetica", "bold");
    doc.setFontSize(20);
    doc.text("Auftrag", 18, 90.5);
    drawCheckbox(pdfOrder.orderType === "lieferschein", "", 56, 90);
    ...
    drawLineField("Mitarbeiter:", pdfOrder.employee || "", 128, 42.5, 24, 44, 24);
    drawLineField("Kunden Nr.:", pdfOrder.customerNo || "", 128, 51.5, 24, 44, 20);
    drawLineField("Kundenbestellnr.:", pdfOrder.customerOrderNo || "", 128, 60.5, 32, 36, 20);
    drawLineField("Kommission:", pdfOrder.commission || "", 128, 69.5, 23, 45, 20);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(8.3);
    doc.text("Liefertermin", 128, 78.5);
    drawLineField("Wunsch:", pdfOrder.deliveryAddress || "", 128, 85.5, 20, 48, 24);
    ...
    lines.forEach((line, idx) => {
      ...
      doc.text((line.description || "").slice(0, 42), 59.5, y);
      doc.text(money(normalizeNumber(line.unitPrice)), 141.4, y);
      doc.text(money(lineTotal(line)), 168.2, y);
    });

    doc.rect(12, 228, 64, 18);
    doc.setFont("helvetica", "bold");
    doc.setFontSize(8.4);
    doc.text("Lieferadresse:", 14, 233);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(8);
    const deliveryLines = doc.splitTextToSize(pdfOrder.deliveryAddress || "", 58).slice(0, 3);
    deliveryLines.forEach((line: string, idx: number) => doc.text(line, 14, 238 + idx * 3.5));

    doc.rect(76, 228, 80, 18);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(8);
    if (pdfOrder.signatureDataUrl) {
      try {
        doc.addImage(pdfOrder.signatureDataUrl, "PNG", 97, 230.2, 38, 7.6);
      } catch {}
    }
    if (pdfOrder.signatureName) {
      doc.setFontSize(7.8);
      doc.text(String(pdfOrder.signatureName).slice(0, 34), 116, 240.0, { align: "center" });
    }
    doc.setFontSize(7.2);
    doc.text("Name/ Unterschrift des Auftraggebers", 116, 244.0, { align: "center" });

    doc.rect(156, 228, 40, 9);
    doc.rect(156, 237, 40, 9);
    doc.setFont("helvetica", "bold");
    doc.setFontSize(8.4);
    doc.text("Summe:", 176, 233.6, { align: "center" });
    doc.text("Abteilung:", 176, 242.5, { align: "center" });
    doc.setFont("helvetica", "normal");
    doc.setFontSize(8.2);
    doc.text(money(orderTotal(pdfOrder)), 176, 236.8, { align: "center" });

    doc.setFontSize(7.5);
    doc.text("Pflichtdaten: Rechnung per E-Mail an:", 12, 253);
    doc.line(66, 253.2, 146, 253.2);
    if (pdfOrder.emailInvoice) {
      doc.text(pdfOrder.emailInvoice.slice(0, 42), 68, 252.2);
    }
    doc.text(
      "Alle Preise sind netto Preise ohne Abzug. Zuzüglich gesetzlich geltender Mehrwertsteuer und Fracht.",
      12,
      259.5,
    );
    doc.text(
  ...
  const buildSingleOrderPdf = async (pdfOrder: Order) => {
    const { jsPDF } = await import("jspdf");
    const doc = new jsPDF("p", "mm", "a4");
    drawOrderPdfPage(doc, pdfOrder);
    return doc;
  };

  const exportCurrentOrder = async () => {
    const doc = await buildSingleOrderPdf(order);
  ...
  const drawDailyReportPdfPage = (doc: any) => {
    ...
    const drawCell = (
      ...
      const lines = doc.splitTextToSize(String(value || ""), width).slice(0, maxLines);
      lines.forEach((line: string, idx: number) => {
        if (align === "center") doc.text(line, x + width / 2, y + idx * lineHeight, { align: "center" });
        else if (align === "right") doc.text(line, x + width, y + idx * lineHeight, { align: "right" });
        else doc.text(line, x, y + idx * lineHeight);
      });
    };
    ...
    [14, 20, 26].forEach((yy) => doc.line(120, yy, 190, yy));
    ...
    doc.rect(table.left, table.top, table.comment - table.left, table.bodyBottom - table.top);
    [table.nr, table.code, table.addr, table.repeat, table.first, table.sum].forEach((x) => doc.line(x, table.top, x, table.bodyBottom));
    doc.line(table.left, table.headerBottom, table.comment, table.headerBottom);
    doc.line(table.addr, 40.5, table.sum, 40.5);

    doc.setFont("helvetica", "normal");
    doc.setFontSize(7.1);
    doc.text("Fax/W/N", 25.1, 39.3);
    doc.text("Uhrzeit", 25.2, 43.5);
    doc.text("NAME UND ADRESSE", 63, 41.4);
    doc.text("PRODUKT", (table.addr + table.sum) / 2, 38.8, { align: "center" });
    doc.text("Nachbestellung", 116, 44.4, { align: "center" });
    doc.text("Erstbestellung", 144, 44.4, { align: "center" });
    doc.text("Summe", 167.4, 42.1, { align: "center" });
    doc.text("Kommentar", 183.3, 41.4, { align: "center" });
    doc.text("Cod.", 183.3, 44.4, { align: "center" });

    for (let i = 0; i <= 10; i += 1) {
      doc.line(table.left, table.rowTop + i * table.rowH, table.comment, table.rowTop + i * table.rowH);
      if (i < 10) {
        const innerTop = table.rowTop + i * table.rowH;
        doc.line(table.code, innerTop + 6.4, table.addr, innerTop + 6.4);
        doc.line(table.code, innerTop + 12.8, table.addr, innerTop + 12.8);
        doc.line(table.addr, innerTop + 12.8, table.repeat, innerTop + 12.8);
        doc.line(table.repeat, innerTop + 12.8, table.first, innerTop + 12.8);
      }
    }

    rows.forEach((row, idx) => {
      const ry = table.rowTop + idx * table.rowH;
      ...
      doc.setFont("helvetica", "normal");
      doc.setFontSize(7.0);
      drawCell(row.code, 24.5, ry + 8.0, 12.5, 3.4, 2, "center");

      const parts = String(row.nameAddress || "").split("\n");
      drawCell(parts[0] || "", 39, ry + 4.6, 61, 3.8, 1);
      drawCell(parts[1] || "", 39, ry + 10.4, 61, 3.8, 1);
      drawCell(parts[2] || "TEL. NR. ODER PERSON", 39, ry + 16.0, 61, 3.8, 1);

      drawCell(row.productRepeat || "", 103, ry + 4.8, 24.8, 3.4, 3);
      drawCell(row.productFirst || "", 131.2, ry + 4.8, 24.8, 3.4, 3);

      if (row.amount) {
        drawCell(money(row.amount), 159.4, ry + 15.0, 16.2, 3.6, 1, "center");
      }

      drawCell(row.comment || "", 178.3, ry + 8.5, 10.4, 3.3, 3, "center");
    });

    const repeatSum = rows.reduce((sum, row) => sum + (row.productRepeat ? row.amount : 0), 0);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(7.4);
    doc.text("SUMME WIEDERHOLUNGSGESCHÄFTE", 13, 244.2);
    doc.text(money(repeatSum), 102, 244.2);
    ...
    const drawSummaryTable = (
      ...
      const w = 86;
      const h = 46;
      const titleBottom = y0 + 7.2;
      const headBottom = y0 + 17.2;
      const col1 = x0 + 8;
      const col2 = x0 + 19;
      const col3 = x0 + 33;
      const col4 = x0 + 47;
      ...
      doc.rect(x0, y0, w, h);
      doc.line(x0, titleBottom, x0 + w, titleBottom);
      doc.line(x0, headBottom, x0 + w, headBottom);
      for (let i = 1; i <= 3; i += 1) doc.line(x0, headBottom + i * 9.75, x0 + w, headBottom + i * 9.75);

      // verticals only below merged title row
      [col1, col2, col3, col4, col5].forEach((x) => doc.line(x, titleBottom, x, y0 + h));

      doc.setFont("helvetica", "normal");
      doc.setFontSize(7);
      doc.text(title, x0 + w / 2, y0 + 4.8, { align: "center" });
      doc.text("Art", x0 + 2.5, y0 + 13.5);
      doc.text("Total", x0 + 26.0, y0 + 13.5, { align: "center" });
      doc.text("Summe", x0 + 74.0, y0 + 13.5, { align: "center" });
      doc.text("Besuche", x0 + 13.0, y0 + 20.1, { align: "center" });
      doc.text("Demos", x0 + 26.0, y0 + 20.1, { align: "center" });
      doc.text("Verkäufe", x0 + 40.0, y0 + 20.1, { align: "center" });
      ...
      rowsLocal.forEach((entry, idx) => {
        const cy = y0 + 24.7 + idx * 9.75;
        doc.text(entry[0], x0 + 2.8, cy);
        doc.text(String(entry[1].visits), x0 + 13.0, cy, { align: "center" });
        doc.text(String(entry[1].demos), x0 + 26.0, cy, { align: "center" });
        doc.text(String(entry[1].sales), x0 + 40.0, cy, { align: "center" });
        doc.text(money(entry[1].amount), x0 + 74.0, cy, { align: "center" });
      });
    };
  ...
  const buildDailyReportPdf = async () => {
    const { jsPDF } = await import("jspdf");
    const doc = new jsPDF("p", "mm", "a4");
    drawDailyReportPdfPage(doc);
    return doc;
  };
  ...
  const sendDailyBatch = async () => {
    ...
    const todaysOrders = liveOrders.filter((o) => o.date === today());
    if (!todaysOrders.length) {
      setStatus("Keine heutigen Aufträge gespeichert");
      return;
    }
    ...
    todaysOrders.forEach((dayOrder) => {
      doc.addPage();
      drawOrderPdfPage(doc, dayOrder);
    });

    const blob = doc.output("blob");
    const name = `Most_Tagesversand_${today()}.pdf`;
    const url = registerGeneratedPdf(name, blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = name;
    a.click();
    setStatus("Tagesversand als gemeinsame PDF erzeugt");
  };
  ...
  const getPlanningCustomerProducts = (customerId: string) => {
    ...
    const mergedProducts = mergeProductSummaries(
      summarizeCustomerProducts(customerOrders),
      summarizeImportedHistoryProducts(importedHistoryMap.get(customerIdentity(customer)) || importedHistoryMap.get(`no:${customer.customerNo.trim()}`) || []),
    );
  ...
  const importBackup = async (file: File) => {
    ...
    for (const importedHistory of parsed.importedHistories || []) await dbPut("importedHistories", importedHistory);
    ...
    setDayPlans(importedDayPlans);
    if (importedDayPlans.length) setDayPlan(importedDayPlans[0]);
    setImportedHistories((await dbGetAll<ImportedCustomerHistory>("importedHistories")).sort((a, b) => String(b.updatedAt || "").localeCompare(String(a.updatedAt || ""))));
    setOrder(emptyOrder(parsed.settings.employeeName, parsed.settings.areaCode));
    setStatus("Backup importiert");
  };


  const normalizeImportedCustomerHistory = (
    rawCustomer: NonNullable<CustomerHistoryJsonPayload["customers"]>[number],
    sourcePdf: string,
    index: number,
  ): ImportedCustomerHistory | null => {
    const customerNo = String(rawCustomer.customer_no || "").trim();
    const company = String(rawCustomer.company || "").trim();
    const street = String(rawCustomer.street || "").trim();
    const zip = String(rawCustomer.zip || "").trim();
    const city = String(rawCustomer.city || "").trim();

    if (!customerNo && !company && !street && !city) return null;

    return {
      id: `imported-${customerNo || company || index}`,
      customerNo,
      company,
      street,
      zip,
      ...
      fax: String(rawCustomer.fax || "").trim(),
      sourcePdf,
      importedAt: new Date().toISOString(),
      updatedAt: new Date().toISOString(),
      products: (((rawCustomer as any).products) || rawCustomer.articles || []).map((article: any) => ({
        articleNo: String(article.article_no || article.articleNo || "").trim(),
        description: String(article.description || "").trim(),
        lastSaleDate: String(article.last_sale_date || article.lastSaleDate || "").trim(),
        lastPrice: normalizeNumber(
          article.last_price ??
            article.lastPrice ??
            article.unit_price ??
            article.unitPrice ??
  ...
  const previewCustomerHistoryJson = async (file: File) => {
    setHistoryImportBusy(true);
    try {
      const parsed = JSON.parse(await file.text()) as CustomerHistoryJsonPayload;
      const sourcePdf = String(parsed.source_pdf || file.name || "JSON-Import");
      const normalizedCustomers = (parsed.customers || [])
        .map((customer, index) => normalizeImportedCustomerHistory(customer, sourcePdf, index))
        .filter(Boolean) as ImportedCustomerHistory[];

      const productMap = new Map<string, Product>();

    (parsed.products || []).forEach((product: any) => {
      const key = String(product.articleNo || product.article_no || "").trim();
      if (!key) return;
    ...
    const normalizedProducts: Product[] = Array.from(productMap.values());

      setHistoryPreview({
        sourcePdf,
        customerCount: normalizedCustomers.length,
        productCount: normalizedCustomers.reduce((sum, customer) => sum + customer.products.length, 0),
        customers: normalizedCustomers,
        products: normalizedProducts,
        sampleCustomers: normalizedCustomers.slice(0, 6),
      });
      setStatus("JSON analysiert. Vorschau bereit.");
    } catch (error) {
      setStatus(error instanceof Error ? error.message : "JSON konnte nicht gelesen werden.");
  ...
  const importCustomerHistoryPreview = async () => {
    if (!historyPreview) {
      setStatus("Keine Import-Vorschau vorhanden");
      return;
    }

    const nextCustomers = [...customers];
    const byId = new Map(customers.map((customer) => [customer.id, customer]));
    const byNo = new Map(customers.filter((customer) => customer.customerNo.trim()).map((customer) => [customer.customerNo.trim(), customer]));
    const byAddress = new Map(
      customers.map((customer) => [
        [customer.company, customer.street, customer.city].map((value) => value.trim().toLowerCase()).join("|"),
        customer,
      ]),
    );

    for (const history of historyPreview.customers) {
      const existing =
        (history.customerNo.trim() && byNo.get(history.customerNo.trim())) ||
        byAddress.get([history.company, history.street, history.city].map((value) => value.trim().toLowerCase()).join("|"));

      const payload: Customer = {
        id: existing?.id || `customer-import-${history.customerNo || history.company}-${Date.now()}`.replace(/\s+/g, "-"),
        customerNo: history.customerNo || existing?.customerNo || "",
        company: history.company || existing?.company || "",
        street: history.street || existing?.street || "",
        zip: history.zip || existing?.zip || "",
        city: history.city || existing?.city || "",
        phone: history.phone || existing?.phone || "",
        fax: history.fax || existing?.fax || "",
        emailInvoice: existing?.emailInvoice || "",
      ...
      if (payload.customerNo.trim()) byNo.set(payload.customerNo.trim(), payload);
      byAddress.set([payload.company, payload.street, payload.city].map((value) => value.trim().toLowerCase()).join("|"), payload);
    }

    const mergedCustomers = Array.from(byId.values());
    for (const customer of mergedCustomers) await dbPut("customers", customer);

    const productMap = new Map(products.map((product) => [product.articleNo || product.description, product]));
    historyPreview.products.forEach((product) => {
      const key = product.articleNo || product.description;
      if (!key) return;
    ...
    for (const product of productMap.values()) {
      if (!product.articleNo && !product.description) continue;
      await dbPut("products", product);
    }

    for (const history of historyPreview.customers) {
      const existingCustomer =
        mergedCustomers.find((customer) => customer.customerNo.trim() && customer.customerNo.trim() === history.customerNo.trim()) ||
        mergedCustomers.find(
          (customer) =>
            [customer.company, customer.street, customer.city].map((value) => value.trim().toLowerCase()).join("|") ===
            [history.company, history.street, history.city].map((value) => value.trim().toLowerCase()).join("|"),
        );

      await dbPut("importedHistories", {
        ...history,
        id: existingCustomer?.id || history.id,
        customerNo: existingCustomer?.customerNo || history.customerNo,
        company: existingCustomer?.company || history.company,
        street: existingCustomer?.street || history.street,
        zip: existingCustomer?.zip || history.zip,
        city: existingCustomer?.city || history.city,
        updatedAt: new Date().toISOString(),
      } satisfies ImportedCustomerHistory);
    }

    setCustomers((await dbGetAll<Customer>("customers")).sort((a, b) => String(a.company || "").localeCompare(String(b.company || ""))));
    setProducts(await dbGetAll<Product>("products"));
    setImportedHistories((await dbGetAll<ImportedCustomerHistory>("importedHistories")).sort((a, b) => String(b.updatedAt || "").localeCompare(String(a.updatedAt || ""))));
    setHistoryPreview(null);
    setStatus("Kundenhistorie importiert");
  };
    ...
    <div className="app-shell">
      <style>{`
        ...
        *{box-sizing:border-box}
        ...
        .tab{
          padding:15px 28px;border:1px solid var(--line);border-radius:18px;background:rgba(255,255,255,.92);cursor:pointer;
          color:var(--text);font-weight:800;transition:.18s ease;font-size:19px;
        }
        ...
        .btn{
          border:1px solid var(--line);background:#fff;border-radius:16px;padding:14px 22px;cursor:pointer;
          display:inline-flex;align-items:center;gap:6px;font-weight:800;transition:.18s ease;font-size:18px
        }
              ...
              <div className="customer-right-sections">
                <div className="box">
                  ...
                  <div className="table-wrap small">
                    <table className="data-table">
                      <thead><tr><th>Art.-Nr.</th><th>Bezeichnung</th><th>Menge</th><th>Preis</th><th>Gesamtpreis</th><th>Verkäufe</th><th>Demos</th></tr></thead>
                      <tbody>
          ...
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="stack" style={{ marginTop: 16 }}>
              ...
              <div className="button-row wrap">
                ...
                <button className="btn secondary" onClick={() => historyImportRef.current?.click()} disabled={historyImportBusy}>
                  {historyImportBusy ? "Analysiere JSON..." : "Kundenhistorie JSON importieren"}
                </button>
                <input
                  ref={importRef}
                  type="file"
                ...
                <input
                  ...
                  onChange={(e) => {
                    const file = e.target.files?.[0];
                    if (file) void previewCustomerHistoryJson(file);
                    e.currentTarget.value = "";
                  }}
                />
              </div>
              ...
              {historyPreview ? (
                <div className="box" style={{ marginTop: 8 }}>
                  <h3>Import-Vorschau Kundenhistorie</h3>
                  <div className="info-list" style={{ marginTop: 10 }}>
                    <div><strong>Quelle:</strong> {historyPreview.sourcePdf}</div>
                    <div><strong>Erkannte Kunden:</strong> {historyPreview.customerCount}</div>
                    <div><strong>Erkannte Produktzeilen:</strong> {historyPreview.productCount}</div>
                    <div><strong>Neue/aktualisierte Produkte:</strong> {historyPreview.products.length}</div>
                  </div>
                  <div className="table-wrap small" style={{ marginTop: 12 }}>
                    <table className="data-table">
                      <thead>
                        <tr><th>Kundennr.</th><th>Firma</th><th>Ort</th><th>Produkte</th></tr>
                      </thead>
                      <tbody>
                        {historyPreview.sampleCustomers.map((customer) => (
                          <tr key={customer.id}>
                            <td>{customer.customerNo || "-"}</td>
                            <td>{customer.company || "-"}</td>
                            <td>{[customer.zip, customer.city].filter(Boolean).join(" ") || "-"}</td>
                  ...
                  <div className="button-row wrap" style={{ marginTop: 12 }}>
                    <button className="btn secondary" onClick={() => setHistoryPreview(null)}>Verwerfen</button>
                    <button className="btn primary" onClick={importCustomerHistoryPreview}>Import bestätigen</button>
                  </div>
                </div>
              ) : null}
