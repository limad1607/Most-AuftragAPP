...
export default function App() {
  ...
  const exportDailyReport = async () => {
    ...
    doc.setFont("helvetica", "normal");
    doc.setFontSize(7.1);
    doc.text("Fax/W/N", 25.3, 39.2);
    doc.text("Uhrzeit", 25.3, 43.4);
    doc.text("NAME UND ADRESSE", 63, 41.3);
    doc.text("PRODUKT", 140, 41.4, { align: "center" });
    doc.text("Nachbestellung", 116, 44.5, { align: "center" });
    doc.text("Erstbestellung", 144, 44.5, { align: "center" });
    doc.text("Summe", 167.2, 43.0, { align: "center" });
    doc.text("Kommentar", 183.3, 41.6, { align: "center" });
    doc.text("Cod.", 183.3, 44.6, { align: "center" });
    ...
    rows.forEach((row, idx) => {
      ...
      doc.setFont("helvetica", "normal");
      doc.setFontSize(7.0);
      drawCell(row.code, 24.5, ry + 8.0, 12.5, 3.3, 2, "center");
      ...
      drawCell(row.comment || "", 178.2, ry + 8.4, 10.6, 3.3, 3, "center");
    });
    ...
    const drawSummaryTable = (
      ...
      [col1, col2, col3, col4, col5].forEach((x) => doc.line(x, y0, x, y0 + h));
      [y0 + 16.75, y0 + 26.5, y0 + 36.25, y0 + 46].forEach((y) => doc.line(x0, y, x0 + w, y));

      doc.setFont("helvetica", "normal");
      doc.setFontSize(7);
      doc.text(title, x0 + w / 2, y0 + 4.8, { align: "center" });
      doc.text("Art", x0 + 2.4, y0 + 13.2);
      doc.text("Total", x0 + 26, y0 + 13.2, { align: "center" });
      doc.text("Summe", x0 + 74, y0 + 13.2, { align: "center" });
      doc.text("Besuche", x0 + 13.0, y0 + 20.0, { align: "center" });
      doc.text("Demos", x0 + 26.0, y0 + 20.0, { align: "center" });
      doc.text("Verkäufe", x0 + 40.0, y0 + 20.0, { align: "center" });
      ...
      rowsLocal.forEach((entry, idx) => {
        ...
        doc.text(money(entry[1].amount), x0 + 74, cy, { align: "center" });
      });
    };
