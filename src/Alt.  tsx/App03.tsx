...
export default function App() {
  ...
  const exportCurrentOrder = async () => {
    ...
    doc.text(`Ansprechpartner: ${order.contactPerson || "-"}`, 14, 46);
    ...
    <div className="app-shell">
      <style>{`
        ...
        *{box-sizing:border-box}
        ...
        .grid-2{max-width:1366px;margin:0 auto;display:grid;grid-template-columns:minmax(0,1.72fr) minmax(260px,.62fr);gap:18px}
        .panel{padding:22px}
        ...
        .btn.setup{background:linear-gradient(135deg,var(--green) 0%,var(--green-dark) 100%);color:#fff;border-color:transparent}
        .btn:hover{transform:translateY(-1px)}

        .form-grid{display:grid;gap:12px}
        .form-grid.four{grid-template-columns:repeat(4,minmax(0,1fr))}
        .form-grid.two{grid-template-columns:repeat(2,minmax(0,1fr))}
        label span{display:block;font-size:13px;margin-bottom:7px;color:var(--muted);font-weight:700}
        input,select,textarea{
          width:100%;padding:11px 13px;border:1px solid var(--line);border-radius:14px;font:inherit;background:#fff;
        ...
        .checks{
          ...
          border:1px solid var(--line);border-radius:18px;padding:14px 18px;margin:18px 0;
        }
        .checks label{
          display:inline-flex;align-items:center;gap:10px;font-weight:700;white-space:nowrap;line-height:1.1
        }
        ...
        .metric{
          ...
          border-radius:16px;padding:12px 14px;display:grid;gap:6px;min-height:unset
        }
        ...
        .totals-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:16px;margin-top:16px}
