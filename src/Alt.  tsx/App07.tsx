...
export default function App() {
  ...
  const exportDailyReport = async () => {
    ...
    const top = 35;
    const headerBottom = 46;
    const bodyBottom = 240;
    const rowTop = 46;
    const rowH = 19.4;

    doc.rect(col.left, top, col.comment - col.left, bodyBottom - top);
    [col.nr, col.code, col.addr, col.repeat, col.first, col.sum].forEach((x) => doc.line(x, top, x, bodyBottom));
    doc.line(col.left, headerBottom, col.comment, headerBottom);
    doc.line(col.addr, 40.5, col.sum, 40.5);

    doc.setFont("helvetica", "normal");
    doc.setFontSize(7.2);
    doc.text("Fax/W/N", 25.2, 39.5);
    doc.text("Uhrzeit", 25.3, 43.6);
    doc.text("NAME UND ADRESSE", 63, 41.4);
    doc.text("PRODUKT", 130, 39.4, { align: "center" });
    doc.text("Nachbestellung", 116, 44.2, { align: "center" });
    doc.text("Erstbestellung", 144, 44.2, { align: "center" });
    doc.text("Summe", 167.4, 41.4, { align: "center" });
    doc.text("Kommentar", 183.2, 41.2, { align: "center" });
    doc.text("Cod.", 183.2, 44.2, { align: "center" });

    for (let i = 0; i <= 10; i += 1) {
      doc.line(col.left, rowTop + i * rowH, col.comment, rowTop + i * rowH);
      if (i < 10) {
        const innerTop = rowTop + i * rowH;
        doc.line(col.code, innerTop + 6.4, col.addr, innerTop + 6.4);
        doc.line(col.code, innerTop + 12.8, col.addr, innerTop + 12.8);
        doc.line(col.addr, innerTop + 12.8, col.repeat, innerTop + 12.8);
        doc.line(col.repeat, innerTop + 12.8, col.first, innerTop + 12.8);
      }
    }
    ...
    rows.forEach((row, idx) => {
      ...
      doc.setFont("helvetica", "normal");
      doc.setFontSize(7.2);
      drawCell(row.code, 24.5, ry + 8.1, 12.5, 3.4, 2, "center");
      ...
      drawCell(row.productRepeat || "", 103, ry + 4.8, 24.5, 3.6, 3);
      drawCell(row.productFirst || "", 131.2, ry + 4.8, 24.5, 3.6, 3);

      if (row.amount) {
        drawCell(money(row.amount), 159.2, ry + 15.5, 16.3, 3.6, 1, "center");
      }

      drawCell(row.comment || "", 178.4, ry + 9.2, 10, 3.5, 3, "center");
    });

    const repeatSum = rows.reduce((sum, row) => sum + (row.productRepeat ? row.amount : 0), 0);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(7.4);
    doc.text("SUMME WIEDERHOLUNGSGESCHÄFTE", 13, 244.4);
    doc.text(money(repeatSum), 102, 244.4);
    ...
    const drawSummaryTable = (
      ...
      doc.line(x0 + 19, y0 + 7, x0 + 19, y0 + h);
      doc.line(x0 + 33, y0 + 7, x0 + 33, y0 + h);
      doc.line(x0 + 47, y0 + 7, x0 + 47, y0 + h);
      doc.line(x0 + 62, y0 + 7, x0 + 62, y0 + h);
      ...
      doc.text("Besuche", x0 + 11.5, y0 + 20.1);
      doc.text("Demos", x0 + 26.8, y0 + 20.1);
      doc.text("Verkäufe", x0 + 40.8, y0 + 20.1);
      ...
      rowsLocal.forEach((entry, idx) => {
        ...
        doc.text(String(entry[1].visits), x0 + 14.3, y, { align: "center" });
        doc.text(String(entry[1].demos), x0 + 28.4, y, { align: "center" });
        doc.text(String(entry[1].sales), x0 + 42.3, y, { align: "center" });
